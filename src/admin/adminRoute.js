...
module.exports = (app, DCQuery, upload) => {
  ...
  app.post('/admin/puzzle-settings/box-tent-state', async (req, res) => {
    let result = await DCQuery.metas.update('boxTentState', req.body.boxTentState);
    if ( result.err ) {
      return res.status(201).json({
        error: result.err
      });
  ...
  app.post('/admin/reset', async (req, res) => {
    ...
    if ( pw && pw == 'carnival_reset' ) {
      try {
        ...
        await fs.remove( path.resolve( __dirname, `../../public/user/uploads`) ); // user uploads reset
        return res.sendStatus(201);
      } catch(e) {
        return res.sendStatus(401);
      }
  ...
  app.post('/admin/zip-path', async (req, res) => {
    ...
    try {
      let folder = path.resolve( __dirname, `../../public/user/uploads/${team}`);
      let output = path.resolve( __dirname, `../../public/user/uploads/${team}.zip`);
      await utils.archive.zip(folder, output);
      res.status(201).json({
        zipPath: `/user/uploads/${team}.zip`
      })
    } catch(e) {
